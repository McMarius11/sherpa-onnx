...
static int32_t CallCallback(JNIEnv *env, jobject callback,
  ...
  //   - invoke([F)I                              (specialized, unboxed return)
  //   - invoke(Ljava/lang/Object;)Ljava/lang/Object;  (erased bridge)
  //
  // The old signature "([F)Ljava/lang/Integer;" does NOT exist — it's a
  // hybrid of specialized param + boxed return that Kotlin never generates.
  //
  ...
  if (invoke_mid) {
    // Fast path: call specialized invoke([F)I directly — no boxing needed.
  ...
  // jfloatArray is a jobject, so it passes through as Object.
  // The Kotlin bridge casts it to FloatArray, calls the specialized
  // invoke, boxes the int result to Integer, and returns it.
